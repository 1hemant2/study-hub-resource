...
const JavaScripts = () => {
    ...
    const [topicDownload, setTopicDownload] = useState("https://drive.google.com/uc?export=download&id=1kV83Ki0_wCO2hqYoVJ_ueMR1tXLLP9kM");
    const getTopicsNameFn = async () => {
        try {
            const data = await SubjectApi({ "subject": "javascript" });
            // console.log(data);
            setTopics(data);
        } catch (error) {
            console.log(error);
    ...
    const topicDetailFn = async (value) => {
        try {
            console.log(value);
            setTopicName(value);
            const data = await TopicDetailsApi("javasript", value);
